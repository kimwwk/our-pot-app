 ...
 * Creates use temp IDs like "temp-xxx" generated by the executor
 *
 * @param entity - The entity type
 * @param entityId - The entity ID (temp-xxx for creates, real ID for updates/deletes)
 * @returns Buffer key for the keyed buffer Map
 */
export function makeBufferKey(entity: EntityType, entityId: string | null): string {
  if (!entityId) {
    // Fallback: generate temp key if somehow entityId is null
...
/**
 * Check if an entity ID is a temp ID (proposal in current changeset)
 */
function isTempId(entityId: string | null): boolean {
  return entityId !== null && entityId.startsWith('temp-');
}
...
async function addToBuffer(
  ...
  // Handle delete with temp ID - remove from buffer instead of creating delete operation
  if (operation === 'delete') {
    ...
    if (entityId && isTempId(entityId)) {
      ...
      console.log('🗑️ [ChangeRequest] Removed proposal from buffer (temp ID delete)', { entityType, entityId, key });
      return;
    }
  }

  // Fetch current data for updates/deletes (only for real IDs)
